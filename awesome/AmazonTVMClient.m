...
-(Response *)anonymousRegister
{
    ...
    if ( [AmazonAnonymousKeyChainWrapper getUidForDevice] == nil) {
        ...
        if ( [response wasSuccessful]) {
            [AmazonAnonymousKeyChainWrapper registerDeviceId:uid andKey:key];
        }
        else {
...
-(Response *)getToken
{
    NSString         *uid = [AmazonAnonymousKeyChainWrapper getUidForDevice];
    NSString         *key = [AmazonAnonymousKeyChainWrapper getKeyForDevice];
    ...
    if ( [response wasSuccessful]) {
        [AmazonAnonymousKeyChainWrapper storeCredentialsInKeyChain:response.accessKey secretKey:response.secretKey securityToken:response.securityToken expiration:response.expirationDate];
    }
    else {
        AMZLogDebug(@"Token Vending Machine responded with Code: [%d] and Messgae: [%@]", response.code, response.message);
    }
